...
FINANCIAL_ITEMS = [
    ...
    },
    {
        ...
        "keywords":  ["이익잉여금", "이익잉여금(결손금)", "결손금", "미처분이익잉여금",
                      "미처리결손금", "미처분결손금", "이월결손금"],
        # 결손금(누적손실) 계열로 매칭되면 부호를 무조건 음수로 보정한다.
        # (일부 보고서는 결손금을 양수 절댓값으로 표기함)
        "negate_keywords": ["결손금", "미처리결손금", "미처분결손금", "이월결손금"],
        "sign":      1,
    },
    # ── 손익계산서 항목 ──────────────────────────────────────────────────
    {
        ...
        "fs_type":   "IS",
        # 키워드는 normalize_label() 적용 후 '정확히' 일치해야 하므로(substring 아님)
        # "매출"을 넣어도 매출원가·매출채권·상품매출 등에는 걸리지 않는다. 일부 연결
        # 포괄손익계산서는 매출 라벨이 "매출액"이 아닌 "매출"이라(예: 2025 HRS코리아),
        # 이게 없으면 본문 손익계산서를 놓치고 종속기업 요약 주석의 "매출액"을 잘못
        # 집는다. "수익" 단독은 여전히 너무 광범위하여 제외.
        "keywords":  ["매출액", "매출", "영업수익", "수익(매출액)", "매출수익"],
        "sign":      1,
    },
    {
        "name":      "매출원가",
    ...
    },
    {
        ...
        "dart_code": None,
        "fs_type":   "CF",
        # 대부분 회사가 감가상각비에 포함시키지만, IFRS 16 채택 후 분리 기재하는
        # 일부 회사가 있어 별도 항목으로 추적한다. 분리 기재된 경우만 값이 들어간다.
        "keywords":  ["사용권자산상각비", "사용권자산상각비용", "사용권자산상각",
